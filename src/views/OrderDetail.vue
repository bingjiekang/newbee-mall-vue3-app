...
<template>
  <div class="order-detail-box">
    ...
    <div class="order-status">
      ...
      <div class="status-item">
        <label>订单编号：</label>
        <span @click="copy(state.detail.orderNo)" style="cursor: pointer">{{ state.detail.orderNo }}</span>
      </div>
      <div class="status-item">
        <label>下单时间：</label>
        <span>{{ timeTransfer(state.detail.createTime) }}</span>
      </div>
    ...
    <template v-slot:desc>
      <template v-for="blockVal in item.blockChainHash" :key="blockVal">
...
import { getOrderDetail, cancelOrder, confirmOrder, payOrder,timeTransfer,hideMiddleString } from '@/service/order'
